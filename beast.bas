...
75 color 8,6,1:SCREEN 5,3,0:: cls : :KEYOFF:DEFINTA-Z: :open"grp:"as#1:S=&h7800 : :forI=0 to (4*64-1+32): READ R$: VPOKE S+I,VAL("&H"+R$):NEXTI:X=100:Y=150 :  H =0: F=0 : E=0 : DX=0 : 
90  pset(0,30): color 9:  for i=0to61  :print#1,chr$(&hc0+RND(1)*6); : next i : pset(0,80): color 8  : for i=0to124:  :print#1,chr$(&hdb+RND(1)*4); : next i
93 LINE(00,120)-(300,182),8,BF: :   PUT SPRITE 0,(X,Y),6,0:PUT SPRITE 1,(X,Y),9,1:   PUT SPRITE 2,(220,0),6,8   
